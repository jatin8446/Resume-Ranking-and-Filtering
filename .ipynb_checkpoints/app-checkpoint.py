...
def index():
    ...
    if request.method == 'POST':
        ...
        for i in range(len(stored_texts)):
            ...
            score = calculate_score(text, job_desc_clean) if stored_job_desc else 0

            matched = match_skills(text, skills)
            total = len(skills)
            
            # Reverting back to your original skill-based percentage!
            percent = (len(matched)/total)*100 if total else 0

            clean_name = name.rsplit('.', 1)[0].replace('_', ' ') if '.' in name else name.replace('_', ' ')
            
            candidate = {
                "name": clean_name,
                "filename": name,
                "percent": round(percent, 2),
                "experience": get_experience_years(text),
